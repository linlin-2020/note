...
官方文档这样解释到：可绘制，可显示响应状态变化的涟漪效应。对于给定状态，纹波的锚固位置可以通过setHotspot(float, float)使用相应的状态属性标识符进行调用来指定。

触摸反馈可绘制对象可能包含多个子层，包括未绘制到屏幕上的特殊蒙版层。通过将XML的android:id值 指定为，可以将单个图层设置为XML的掩码R.id.mask。在运行时，可以使用设置单个层作为遮罩，setId(..., android.R.id.mask)或者可以使用替换现有的遮罩层setDrawableByLayerId(android.R.id.mask, ...)。
...
1. 如果设置了遮罩层，则纹波效果将在该层上遮盖，然后再将其绘制在其余子层的复合层上。

2. 如果未设置遮罩层，则纹波效果将针对子层的复合遮罩。
...
3. 如果未指定任何子层或蒙版，并且将波纹设置为视图背景，则波纹将绘制在视图层次结构中第一个可用的父背景之上。在这种情况下，绘图区域可能会延伸到Drawable边界之外。
